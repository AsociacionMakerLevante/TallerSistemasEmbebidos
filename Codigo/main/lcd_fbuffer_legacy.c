...
*/
void lcd_buffer_drawtext(uint8_t *buffer, uint16_t x, uint16_t y, uint16_t width, uint16_t height, char *texto)
{
    ...
    for (uint16_t numero_caracter = 0; numero_caracter < strlen(texto); numero_caracter++)
    {
        ...
        */
        ...
        caracter = ((height * width) >> 3) * (texto[numero_caracter] - ' ');
        // Puntero al inicio del carácter a escribir dentro del array.
        escribirChar = fuente + caracter;
        // Dibujamos el carácter.
        lcd_fbuffer_imagen(buffer, x_actual, y_actual, width, height, escribirChar);
        x_actual += width;
    }
}
/*
